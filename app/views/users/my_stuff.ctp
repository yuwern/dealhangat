...
<div class="js-mystuff-tabs">
    <ul class="clearfix">
        <?php if($this->Auth->user('user_type_id') == ConstUserTypes::Company):?>    
            ...
            <li><?php echo $this->Html->link(__l('My Account'), array('controller' => 'companies', 'action' => 'edit', $user['Company']['id']), array('title' => 'My Account', 'rel' => 'address:/' . __l('My_Account'))); ?></li>
        <?php else: ?>
        ...
        <?php if($this->Auth->sessionValid() && $this->Html->isAllowed($this->Auth->user('user_type_id'))):?>
              <li><?php  echo $this->Html->link(sprintf(__l('My %s'), Configure::read('site.name')), array('controller' => 'deal_users', 'action' => 'index'), array('title' => 'My Purchases', 'rel' => 'address:/' . __l('My_Purchases')));?></li>
