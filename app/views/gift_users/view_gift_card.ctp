...
<div class="giftUser">
	...
	<?php endif; ?>
		<div class="clearfix <?php echo ($this->request->params['isAjax'])?'gift-card':'giftcard-bg';?>">
            ...
            <?php if(!$this->Auth->user('id')): ?>
				<div class="gift-login login-right-block">
                     <?php echo $this->Html->link(__l('Login'), '#', array('title' => __l('Login to Site'), 'class' => "register-link js-toggle-show {'container':'js-gift-card-login', 'hide_container': 'js-gift-card-register'}"));?>
					 <?php echo $this->Html->link(__l('Register'), '#', array('title' => __l('Register to Site'), 'class' => "register-link js-toggle-show {'container':'js-gift-card-register', 'hide_container':'js-gift-card-login'}"));?>
            ...
            <?php elseif(!$giftUser['GiftUser']['is_redeemed'] && $this->Auth->user('user_type_id') != ConstUserTypes::Admin && $giftUser['GiftUser']['friend_mail'] == $this->Auth->user('email')): ?>
				<div class="reedeem-block">
					 <?php echo $this->Html->link(__l('Redeem'), array('controller'=> 'gift_users', 'action'=>'redeem', $giftUser['GiftUser']['coupon_code']), array('title'=>__l('Redeem'),'escape' => false));?>
				 </div>
            <?php endif; ?>
			</div>
		</div>
