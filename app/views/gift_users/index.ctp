...
<?php if(empty($this->request->params['named']['type']) && empty($this->request->params['named']['sort'])): ?>
    ...
    <div class="js-tabs">
        <ul class="clearfix">
            <li><?php echo $this->Html->link(sprintf(__l('Received (%s)'),$received),array('controller'=> 'gift_users', 'action'=>'index', 'type' => 'received'),array('title' => 'Received Gift Cards')); ?></li>
            <li><?php echo $this->Html->link(sprintf(__l('Sent (%s)'),$sent),array('controller'=> 'gift_users', 'action'=>'index', 'type' => 'sent'), array('title' => 'Sent Gift Cards')); ?></li>
            <li><?php echo $this->Html->link(sprintf(__l('All (%s)'),($sent+$received)),array('controller'=> 'gift_users', 'action'=>'index', 'type' => 'all'), array('title' => 'All Gift Cards')); ?></li>
        </ul>
    </div>
<?php else: ?>
    <div class="giftUsers index js-response">
    <div class="clearfix add-block">
        <?php echo $this->Html->link(__l('Buy a Gift Card'), array('controller' => 'gift_users', 'action' => 'add'), array('class' => 'buy-gift','title'=>__l('Buy a gift card'))); ?>
    ...
    <table class="list">
        <tr>
            ...
            <?php if (!empty($this->request->params['named']['type']) && $this->request->params['named']['type'] == 'received'): ?>
                <th class ="dl"><div  class="js-pagination"><?php echo $this->Paginator->sort(__l('Received From'),'user_id');?></div></th>
            <?php endif; ?>
            <th class ="dl"><div  class="js-pagination"><?php echo $this->Paginator->sort(__l('Coupon Code'),'coupon_code');?></div></th>
