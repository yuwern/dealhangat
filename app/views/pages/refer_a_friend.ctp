...
<p><?php echo sprintf(__l('Get %s in %s Bucks when someone you invite gets their first %s. There is no limit on how much you can earn!'),$this->Html->siteCurrencyFormat($this->Html->cCurrency(Configure::read('user.referral_amount'), false)),Configure::read('site.name'),Configure::read('site.name')); ?></p>
