...
class UsersController extends AppController
{
        ...
        if (!empty($this->request->data['User']['openid']) && preg_match('/^http?:\/\/+[a-z]/', $this->request->data['User']['openid'])) {
            ...
            if (!empty($this->request->data)) {
                ...
                if (!empty($this->request->data['City']['name'])) {
                    $this->request->data['UserProfile']['city_id'] = !empty($this->request->data['City']['id']) ? $this->request->data['City']['id'] : $this->User->Company->City->findOrSaveAndGetId($this->request->data['City']['name']);
                }
                if (!empty($this->request->data['State']['name'])) {
                    $this->request->data['UserProfile']['state_id'] = !empty($this->request->data['State']['id']) ? $this->request->data['State']['id'] : $this->User->Company->State->findOrSaveAndGetId($this->request->data['State']['name']);
                }
    ...
    public function _facebook_login($id = null)
    {
        ...
        if (empty($user)) {
            ...
            //$this->request->data['User']['referred_by_user_id'] = !empty($referrer_id) ? $this->Cookie->read('referrer') : '';
            if ($this->Session->read('gift_user_id')) {
                $this->request->data['User']['gift_user_id'] = $this->Session->read('gift_user_id');
                $this->Session->delete('gift_user_id');
            }
    ...
    public function admin_stats()
    {
        ...
        $rowspan_ct = 0;
        if (Configure::read('facebook.is_enabled_facebook_connect') != 1) {
            $rowspan_ct++;
        }
        if (!Configure::read('user.is_enable_openid') && !Configure::read('user.is_enable_gmail_openid') && !Configure::read('user.is_enable_yahoo_openid')) {
            $rowspan_ct++;
        ...
        $models[] = array(
            'User' => array(
                ...
                'link' => array(
                    'controller' => 'users',
                    'action' => 'index',
                    'main_filter_id' => ConstMoreAction::Twitter
                ) ,
                'conditions' => array(
		...
		$models[] = array(
            'User' => array(
                ...
                'link' => array(
                    'controller' => 'users',
                    'action' => 'index',
                    'main_filter_id' => ConstMoreAction::Foursquare
                ) ,
                'conditions' => array(
            ...
            $models[] = array(
                'User' => array(
                    ...
                    'link' => array(
                        'controller' => 'users',
                        'action' => 'index',
                        'main_filter_id' => ConstMoreAction::OpenID
                    ) ,
                    'conditions' => array(
        ...
        $models[] = array(
            'User' => array(
                ...
                'link' => array(
                    'controller' => 'users',
                    'action' => 'index',
                    'main_filter_id' => ConstMoreAction::Gmail
                ) ,
                'conditions' => array(
        ...
        $models[] = array(
            'User' => array(
                ...
                'link' => array(
                    'controller' => 'users',
                    'action' => 'index',
                    'main_filter_id' => ConstMoreAction::Yahoo
                ) ,
                'conditions' => array(
